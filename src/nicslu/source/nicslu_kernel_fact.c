...
int NicsLU_Factorize(SNicsLU *nicslu)
{
	...
	for (i=0; i<n; ++i)
	{
		...
		/* Print L and U indices for the current row */
		printf("Row %u:\n", i);
		
		/* Print ll and ul values */
		printf("  ll: %u, ul: %u\n", ll, ul);
		
		/* Print U indices and data */
		printf("  U indices: ");
		u_row_index_j = (uint__t *)(((byte__t *)lu_array) + up[i]);
		for (q = 0; q < ul; ++q)
		{
			printf("%u ", u_row_index_j[q]);
		}
		printf("\n");
		
		/* Print L indices and data */
		printf("  L indices: ");
		row_index = (uint__t *)(((byte__t *)lu_array) + up[i] + ul * g_sp);
		for (q = 0; q < ll; ++q)
		{
			printf("%u ", row_index[q]);
		}
		printf("\n");
	}
	printf("\n");
...
int NicsLU_Factorize2(SNicsLU *nicslu)
{
    /* ... 之前的代码保持不变 ... */
	real__t *ax;
	uint__t *ai, *ap;
    ...
    for (i=0; i<n; ++i)
    {
        ...
        /*禁用换主元操作：直接使用当前的主元*/
        diagcol = p[i]; /*直接使用当前的主元*/
        pivot = x[diagcol]; /*假设主元是对角线元素的值*/

        /*更新 up, ux, lp, lx, ulen, llen*/
        ll = llen[i] = n-top;
        ul = ulen[i];
        ...
        /*log the pivoting*/
        p[i] = diagcol; /*直接更新主元信息*/
        pinv[diagcol] = i;

        /*prune*/
        _I_NicsLU_Prune(pend, ll, ulen, pinv, diagcol, row_index, up, lu_array);
    ...
    //     for (i=0; i<ll; ++i)
    //     {
    //         printf("%u ", row_index[i]); /*直接输出U的索引，不进行换主元操作*/
    //     }
    //     printf("\n");
    }
